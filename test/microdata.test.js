...
describe('microdata', function () {
  ...
  describe('#parse()', function () {
    ...
    it('should return 2 items and one child when parsing test.html', function () {
      var result = microdata.parse(data);
      assert.equal(result.length, 2);
      assert.equal(result[0].children.length, 1);
    });
    it('should have a title property on the first child when parsing test.html', function () {
      var result = microdata.parse(data);
